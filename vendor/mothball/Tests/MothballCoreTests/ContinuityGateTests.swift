...
final class ContinuityGateTests: XCTestCase {
    ...
    func test_unsealedBindings_block() {
        ...
        XCTAssertEqual(
            ContinuityGate.evaluate(.bindings([binding])),
    ...
    func test_emptyBindings_blockRatherThanPass() {
        XCTAssertEqual(
            ContinuityGate.evaluate(.bindings([])),
    ...
    func test_sealed_allows() {
        ...
        XCTAssertEqual(ContinuityGate.evaluate(.sealed(bundle)), .allow)
    }
    ...
    func test_manifestTags_areDistinctPerState() {
        let tags = [
            ...
            ContinuityAssessment.bindings([]).manifestTag,
            ContinuityAssessment.sealed(.init(stagingRoot: URL(fileURLWithPath: "/tmp"), sessions: [])).manifestTag,
