...
function readText(path) {
  ...
  try {
    ...
    if (Number(data.length) > 32 * 1024 * 1024) {
      throw new Error(t("scan_result.json 크기가 안전 상한을 초과했습니다."));
    }
...
function writeText(path, text) {
  ...
  if (!ok) throw new Error(t("리포트를 안전하게 기록하지 못했습니다: ") + path);
}
function esc(v) {
...
function investigateLinks(row) {
  ...
  const context = esc(label || t("항목"));
  if (vtHash) {
    links.push(localized`<a href="https://www.virustotal.com/gui/file/${urlEncode(vtHash)}" target="_blank" rel="noopener noreferrer" aria-label="VirusTotal에서 ${context} 조사">VT</a>`);
  } else if (row.remoteAddress) {
    links.push(localized`<a href="https://www.virustotal.com/gui/ip-address/${urlEncode(row.remoteAddress)}" target="_blank" rel="noopener noreferrer" aria-label="VirusTotal에서 ${context} 조사">VT IP</a>`);
  }
  if (label) {
    links.push(localized`<a href="https://www.google.com/search?q=${urlEncode(label + " malware")}" target="_blank" rel="noopener noreferrer" aria-label="Google에서 ${context} 검색">Google</a>`);
  }
  return links.length ? links.join(" · ") : "";
}
function table(rows, fields) {
  if (!rows || !rows.length) return t("<p class='muted'>표시할 항목이 없습니다.</p>");
  return localized`<table><thead><tr>${fields.map(f => localized`<th scope="col">${esc(f)}</th>`).join("")}<th scope="col">조사</th></tr></thead><tbody>` +
    rows.map(r => localized`<tr class='risk-${esc(r.risk || "")}'>${fields.map(f => localized`<td>${esc(r[f])}</td>`).join("")}<td>${investigateLinks(r)}</td></tr>`).join("") +
    "</tbody></table>";
}

const requestedLanguage = env("PCH_LANG", "en").replace(/_/g, "-").split("-")[0].toLowerCase();
const reportLanguage = ["ko", "en", "ja"].indexOf(requestedLanguage) >= 0 ? requestedLanguage : "en";
let fragments = {};
try {
  const supplied = env("PCH_REPORT_STRINGS", "");
  if (supplied) fragments = JSON.parse(supplied);
  else {
    const args = ObjC.deepUnwrap($.NSProcessInfo.processInfo.arguments);
    const script = args.find(a => /report\.jxa\.js$/.test(a)) || "scripts/report.jxa.js";
    const root = script.replace(/scripts\/report\.jxa\.js$/, "");
    fragments = JSON.parse(readText(root + "data/report_i18n/" + reportLanguage + ".json")).native_fragments || {};
  }
} catch (_) { /* Preserve diagnostics if a source-checkout catalog is unavailable. */ }
var fragmentKeys = Object.keys(fragments).sort((a, b) => b.length - a.length);
function t(literal) {
  // Single-pass replacement avoids translating a replacement a second time.
  if (!fragmentKeys || !fragmentKeys.length) return literal;
  const pattern = new RegExp(fragmentKeys.map(escapeRegex).join("|"), "g");
  return literal.replace(pattern, key => fragments[key]);
}
function localized(parts, ...values) {
  return parts.map((part, index) => t(part) + (index < values.length ? values[index] : "")).join("");
}
...
if (!scan.summary) throw new Error(t("scan_result.json에 summary가 없습니다."));
...
const actions = overall === "danger"
  ? [t("의심 항목을 바로 삭제하지 말고 프로그램 이름과 경로를 확인하세요."), t("백신으로 전체 검사를 실행하세요."), t("민감한 브라우저 세션을 닫은 뒤 조사하세요.")]
  : overall === "warning"
    ? [t("확인 항목의 프로그램 이름, 게시자, 설치일을 대조하세요."), t("알 수 없는 항목은 검색과 VirusTotal 리포트 링크로 맥락을 확인하세요."), t("정밀 검사를 실행해 유휴 CPU 사용량을 확인하세요.")]
    : overall === "incomplete"
      ? [t("완료하지 못한 필수 수집기를 확인하세요."), t("권한 또는 시간 제한 문제를 해결한 뒤 다시 검사하세요."), t("빈 결과를 정상으로 해석하지 마세요.")]
      : [t("현재 수집 범위에서 즉시 조치가 필요한 항목이 보이지 않습니다."), t("보안 업데이트를 최신 상태로 유지하세요."), t("느림이나 팬 소음이 계속되면 다시 검사하세요.")];
...
const collectionHtml = localized`<div class="panel">
  <h2>검사 범위</h2>
...
const vtHtml = vt.enabled ? localized`<div class="panel">
  <h2>VirusTotal 조회</h2>
  <p class="muted">이번 검사 조회 ${esc(vt.callsThisScan || 0)}건 · 캐시 유지 ${esc(vt.cacheHours || 0)}시간</p>
  ${vtSkipped > 0 ? localized`<div class="share">이번 검사에서 ${esc(vtSkipped)}건은 VT 쿼터 소진으로 확인하지 못했습니다. 위험 판정과는 무관하며, 다음 검사에서 이어서 확인됩니다.</div>` : ""}
</div>` : "";
const storage = s.storage || {};
const storageVolume = storage.volume || {};
const accessHtml = (storage.accessIssues || []).length
  ? localized`<h3>Full Disk Access 확인 필요</h3>
...
const runtimeHtml = (storage.runtimeSignals || []).length
  ...
  : "";
const storageHtml = storage.volume ? localized`<div class="panel">
  ...
  ${(storage.reviewCandidates || []).length ? t("<h3>삭제 전 확인</h3>") : ""}
  ...
  ${(storage.applications || []).length ? t("<h3>설치 앱 크기 및 제거 검토</h3>") : ""}
  ${(storage.applications || []).length ? table(storage.applications || [], ["risk","kind","label","sizeGB","action","note","path"]) : ""}
  ${(storage.simulatorDevices || []).length ? t("<h3>Simulator 기기별 보존 상태</h3>") : ""}
...
const shareNotice = redacted
  ? t("<div class=\"share redacted\">공유용 리포트입니다. PC 이름, 사용자 이름, 홈 디렉터리 경로를 자동으로 가렸습니다. 공유 전 내용을 한 번 더 확인하세요.</div>")
  : t("<div class=\"share\">도움을 요청하려고 리포트를 공유할 때는 PC 이름, 사용자 이름, 경로에 포함된 개인 정보를 먼저 가리세요.</div>");
const html = localized`<!doctype html><html lang="${reportLanguage}" dir="ltr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Modore 결과</title><style>${css}</style></head><body><main id="main"><div class="container">
<h1>🩺 Modore${redacted ? t(" 공유용") : ""} 결과</h1>
...
<div class="panel"><h2>다음 행동</h2><ol>${actions.map(a => localized`<li>${esc(a)}</li>`).join("")}</ol>${shareNotice}</div>
${collectionHtml}
${vtHtml}
...
<h2>주요 발견 사항</h2>${findings.length ? findings.map(f => localized`<div class="finding ${esc(f.level)}"><b>${esc(f.title)}</b><br>${esc(f.detail)}</div>`).join("") : t("<p class='muted'>주의가 필요한 항목이 발견되지 않았습니다.</p>")}
${storageHtml}
...
console.log(t("HTML 리포트 생성: ") + outputPath);
