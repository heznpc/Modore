    ...
    static func make(
        ...
        guard content.summary != nil || content.storage != nil else {
            return IncidentAssessment(
                kind: .noResult,
                title: L10n.text("아직 판단할 검사 결과가 없습니다"),
                detail: L10n.text("앱을 열면 Modore가 자동으로 실행 프로세스, 네트워크, 자동 실행, 보호 상태와 저장공간 변화를 함께 확인합니다."),
                impact: L10n.text("검사 전에는 현재 Mac의 이상 징후나 검사 범위를 판단하지 않습니다."),
                value: L10n.text("정밀 검사 필요"),
        ...
        if content.securityHasDanger {
            ...
            return IncidentAssessment(
                kind: .securityDanger,
                title: first?.title ?? L10n.text("즉시 확인할 위험 신호가 있습니다"),
                detail: first?.detail ?? L10n.text("보안 진단에서 위험 등급 신호를 확인했습니다."),
                impact: L10n.text("알 수 없는 상주 작업이나 외부 통신이 계속될 수 있으므로 삭제 전에 프로세스와 경로를 확인해야 합니다."),
                value: L10n.text("위험"),
        ...
        if let cpu = sustainedCPU(in: content) {
            let window = cpu.row.windowSeconds > 0 ? L10n.format("%@초 관찰에서 ", String(describing: cpu.row.windowSeconds)) : L10n.text("관찰 구간에 ")
            let path = cpu.path.isEmpty ? "" : L10n.format(" 실행 경로: %@", String(describing: cpu.path))
            return IncidentAssessment(
                kind: .sustainedCPU,
                title: L10n.format("%@가 CPU 한 코어 이상을 계속 사용합니다", String(describing: cpu.displayName)),
                detail: String(
                    format: L10n.text("%@%@ 프로세스가 평균 %.1f%%를 사용했습니다.%@"),
                    window,
                    cpu.row.name,
                    cpu.row.cpuPercent,
                    path
                ),
                impact: L10n.text("발열, 배터리 소모와 다른 앱의 응답 지연이 계속될 수 있습니다."),
        ...
        if let storage = content.storage, storage.risk == "danger" {
            return IncidentAssessment(
                ...
                title: L10n.text("저장공간이 임계 수준입니다"),
                detail: String(format: L10n.text("시동 볼륨에 %.1fGB가 남아 있고 사용률은 %.0f%%입니다."), storage.freeGB, storage.usePercent),
                impact: L10n.text("빌드 실패, swap 증가, 앱 응답 지연과 임시파일 생성 실패가 이어질 수 있습니다."),
                value: String(format: L10n.text("%.1fGB 남음"), storage.freeGB),
        ...
        if let coverage = content.collectionCoverage, !coverage.complete {
            ...
            let missing = labels.isEmpty ? L10n.text("일부 필수 수집기") : labels
            return IncidentAssessment(
                kind: .collectionIncomplete,
                title: L10n.text("안전 여부 판단을 보류했습니다"),
                detail: L10n.format("%@을 완료하지 못했습니다. 비어 있는 결과를 정상으로 해석하지 않습니다.", String(describing: missing)),
                impact: L10n.text("확인하지 못한 영역의 프로세스, 연결 또는 자동 실행 항목이 결과에서 빠졌을 수 있습니다."),
        ...
        if content.collectionCoverage == nil {
            return IncidentAssessment(
                kind: .collectionIncomplete,
                title: L10n.text("검사 범위를 확인할 수 없습니다"),
                detail: L10n.text("이전 형식의 결과에는 수집 성공 여부가 기록되지 않았습니다. 새 검사 전에는 정상 판정을 사용하지 않습니다."),
                impact: L10n.text("결과가 비어 있어도 실제 항목이 없었던 것인지 수집에 실패한 것인지 구분할 수 없습니다."),
                value: L10n.text("판단 보류"),
                symbol: "questionmark.shield"
            )
        }
        ...
        if let browser = content.storage?.browserAutomation, browser.needsAttention {
            return IncidentAssessment(
                ...
                title: browser.verdict == "orphaned"
                    ? L10n.text("소유 작업을 찾지 못한 자동화가 있습니다")
                    : L10n.text("기본 Chrome과 자동화가 충돌할 수 있습니다"),
                detail: browser.note,
                impact: L10n.text("일반 Chrome이 열리지 않거나 임시 프로필과 code-sign clone이 계속 남을 수 있습니다."),
                value: L10n.format("%@개 루트", String(describing: browser.rootCount)),
                symbol: "rectangle.on.rectangle"
            )
        }

        if let change = storageChange, change.freeSpaceComparable, change.consumedGB >= 8 {
            let cause = change.primaryCause.map { L10n.format(" 가장 크게 함께 증가한 후보는 %@입니다.", String(describing: $0.label)) } ?? ""
            return IncidentAssessment(
                kind: .storageDrop,
                title: L10n.text("저장공간이 빠르게 줄었습니다"),
                detail: String(format: L10n.text("직전 검사보다 %.1fGB 감소했습니다.%@"), change.consumedGB, cause),
                impact: L10n.text("같은 생성원이 실행 중이면 정리 뒤에도 공간이 다시 줄어들 수 있습니다."),
        ...
        if let first = content.securityAttentionFindings.first {
            return IncidentAssessment(
                ...
                title: first.title,
                detail: first.detail,
                impact: L10n.text("현재 정보만으로 악성 여부를 확정하지 않으며 실행 경로와 설치 맥락을 먼저 대조해야 합니다."),
                value: L10n.text("확인 필요"),
                symbol: "info.circle"
            )
        }
        ...
        if let signal = content.storage?.attentionRuntimeSignals.first {
            return IncidentAssessment(
                ...
                title: L10n.text("개발 작업이 백그라운드에서 계속 실행 중입니다"),
                detail: "\(signal.label): \(signal.note.isEmpty ? signal.action : signal.note)",
                impact: L10n.text("캐시, Simulator 데이터 또는 임시 브라우저 파일이 다시 생성될 수 있습니다."),
        ...
        return IncidentAssessment(
            kind: .clear,
            title: L10n.text("현재 뚜렷한 이상 징후가 없습니다"),
            detail: L10n.format("필수 수집기 %@/%@개가 완료됐고 위험·확인 신호가 없습니다.", String(describing: coverage?.completedRequiredCount ?? 0), String(describing: coverage?.requiredCount ?? 0)),
            impact: L10n.text("이 판단은 표시된 검사 시점과 수집 범위 안에서만 유효합니다."),
            value: L10n.text("범위 내 정상"),
