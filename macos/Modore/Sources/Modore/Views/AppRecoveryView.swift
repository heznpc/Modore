...
struct AppRecoveryView: View {
        ...
        VStack(alignment:.leading,spacing:20) {
            HStack {
                VStack(alignment:.leading,spacing:6) {
                    Text(L10n.text("실행 중인 앱")).font(.system(size:30,weight:.bold))
                    Text(L10n.text("앱을 선택해 작업을 확인하고 다시 시작하세요.")).foregroundStyle(.secondary)
                }
                Spacer()
                Button(L10n.text("새로 고침")) { refresh() }.disabled(busy)
            }
            HStack(alignment:.top,spacing:24) {
                ...
                VStack(spacing:18) {
                    if let app=apps.first(where:{$0.id == highlighted}) {
                        ...
                        Button { focus(app) } label: { Label(L10n.text("앱 앞으로 가져오기"),systemImage:"arrow.up.forward.app").frame(maxWidth:.infinity).padding(8) }.buttonStyle(.borderedProminent).tint(.teal).disabled(busy)
                        Button { selected=app } label: { Label(L10n.text("종료 후 다시 열기…"),systemImage:"arrow.clockwise").frame(maxWidth:.infinity).padding(8) }.buttonStyle(.bordered).disabled(busy)
                        Text(L10n.text("저장 요청이 나오면 앱에서 처리하세요.")).font(.caption).foregroundStyle(.secondary)
                    } else {
                        Image(systemName:"app.dashed").font(.system(size:52)).foregroundStyle(.teal)
                        Text(L10n.text("앱 선택")).font(.title2.bold())
                        Text(L10n.text("왼쪽에서 확인할 앱을 선택하세요.")).foregroundStyle(.secondary)
                    }
                    Spacer()
            ...
            if busy { ProgressView(L10n.text("앱의 정상 종료와 재실행을 확인하고 있습니다…")) }
            if !message.isEmpty { Text(message).textSelection(.enabled) }
            Text(L10n.text("메모리는 측정된 프로세스의 상주량입니다. 미측정 앱과 별도 보조 프로세스는 합산하지 않습니다.")).font(.caption).foregroundStyle(.secondary)
        }.padding(24).frame(maxWidth:.infinity,maxHeight:.infinity).interactiveDismissDisabled(busy)
        .onAppear { refresh() }
        .confirmationDialog(L10n.format("%@을 다시 시작할까요?", selected?.name ?? L10n.text("앱")),isPresented:Binding(get:{selected != nil},set:{if !$0 {selected=nil}}),titleVisibility:.visible) {
            Button(L10n.text("정상 종료 후 다시 열기")) { if let app=selected { Task { await restart(app) } };selected=nil }
            Button(L10n.text("취소"),role:.cancel) { selected=nil }
        } message: { Text(L10n.text("진행 중인 작업이 중단될 수 있습니다. 앱이 저장 확인 등으로 종료하지 않으면 강제 종료하지 않고 기다림을 끝냅니다.")) }
    }
    ...
    private func memoryLabel(_ app:RestartableApp) -> String {
        app.memory.map { ByteCountFormatter.string(fromByteCount:Int64(clamping:$0),countStyle:.memory) } ?? L10n.text("미측정")
    }
    private func focus(_ item:RestartableApp) {
        guard let app=NSRunningApplication(processIdentifier:item.id),app.launchDate==item.launched,app.bundleURL==item.url else {message=L10n.text("앱 상태가 바뀌었습니다. 다시 확인하세요.");refresh();return}
        if !app.activate(options:[.activateIgnoringOtherApps]) { message=L10n.text("앱을 앞으로 가져오지 못했습니다.") }
    }
    ...
    private func restart(_ item:RestartableApp) async {
        ...
        do {
            guard let app=NSRunningApplication(processIdentifier:item.id), app.launchDate==item.launched,
                  app.bundleURL==item.url, FilesystemIdentity.directory(at:item.url)==item.identity else {throw RetirementError(L10n.text("앱의 대상 상태가 바뀌었습니다. 목록을 다시 확인하세요."))}
            ...
            guard app.terminate() else {throw RetirementError(L10n.text("앱이 정상 종료 요청을 받지 못했습니다."))}
            for _ in 0..<60 {
                if app.isTerminated {break}
                try await Task.sleep(nanoseconds:500_000_000)
            }
            guard app.isTerminated else {throw RetirementError(L10n.text("앱이 아직 열려 있습니다. 저장 요청을 처리한 뒤 다시 시도하세요. 강제 종료하지 않았습니다."))}
            guard FilesystemIdentity.directory(at:item.url)==item.identity else {throw RetirementError(L10n.text("종료 후 앱 파일이 바뀌어 재실행하지 않았습니다."))}
            let launched: NSRunningApplication = try await withCheckedThrowingContinuation { continuation in
                NSWorkspace.shared.openApplication(at:item.url,configuration:NSWorkspace.OpenConfiguration()) { app,error in
                    ...
                    else {continuation.resume(throwing:error ?? RetirementError(L10n.text("앱 재실행 응답 없음")))}
                }
            }
            ...
            message=L10n.format("%@을 다시 열었습니다. 메모리·디스크 전후 상태를 기록했습니다.", item.name)
        ...
        do {try save()} catch {message += "\n" + L10n.format("기록 저장 실패: %@", error.localizedDescription)}
    }
}
