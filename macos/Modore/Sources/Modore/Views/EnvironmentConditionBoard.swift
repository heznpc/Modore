        ...
        return VStack(alignment:.leading,spacing:12) {
            HStack {
                ...
                Toggle(isOn:Binding(get:{required.contains(platform)},set:{if $0 {required.insert(platform)} else {required.remove(platform)}})) {
                    Label("유지",systemImage:required.contains(platform) ? "pin.fill" : "pin").padding(.vertical,5).padding(.horizontal,3)
                }.toggleStyle(.button).help("\(platform) 유지 조건 · 이번 검토에 적용, 저장은 자동 정리·유지 조건에서")
                    .accessibilityLabel("\(platform) 유지 조건")
            }
            ...
            if devices.isEmpty {
                Button(action:prepare) { Label("기기 준비",systemImage:"plus.circle") }.buttonStyle(.plain).foregroundStyle(.teal)
            } else {
                Label(affected ? (remaining.isEmpty ? "정리 후 기기 없음" : "정리 후 남는 기기") : "사용 가능",systemImage:affected ? "exclamationmark.circle" : "checkmark.circle.fill")
...
struct EnvironmentConditionChips: View {
    ...
    @ViewBuilder private var chips: some View {
        chip(running ? "실행 중" : item.state == "Shutdown" ? "꺼짐" : "\(item.state == "Ready" ? "설치됨" : item.state == "Mounted" ? "연결됨" : item.state)",running ? "play.fill" : "circle",running ? .teal : .secondary)
        if connections > 0 { chip("작업 \(connections)개 연결","link",.orange) }
        if !item.project.isEmpty { chip(URL(fileURLWithPath:item.project).lastPathComponent,"folder",.teal) }
        if required || item.warnings.contains(where:{$0.contains("유지 표시") || $0.hasPrefix("프로젝트 요구:")}) { chip("유지 조건","pin.fill",.orange) }
        if !item.invariant.isEmpty { chip("확인 필요","lock",.orange) }
    ...
    private var effect:String {
        ...
        case "volume": return "연결 해제"
        default: return "종료 · 파일 유지"
        }
    }
