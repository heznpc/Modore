...
- **Storage pressure becomes an in-app recovery flow**: the Mac app keeps a live warning below 20GB free (urgent below 5GB), opens a safe-cache-first plan aimed at restoring 20GB of headroom, shows every exact target, and runs the ready set after one explicit approval. It stops as soon as the goal is reached and re-reads real filesystem free space after each step; apps, models, Simulator devices, and services remain individual decisions.
